    ...
    return (
      <div className="space-y-4">
        <div className="card-base p-3 animate-pulse">
          <div className="flex gap-2">{[...Array(6)].map((_, i) => <div key={i} className="h-7 w-16 bg-[var(--btn-regular-bg)] rounded-lg" />)}</div>
        </div>
          ...
          {[...Array(5)].map((_, i) => (
            <div key={i} className="mb-6 last:mb-0">
              <div className="flex flex-row w-full items-center h-[3.75rem]">
                ...
                <div className="w-[15%] md:w-[10%] flex justify-center"><div className="h-3 w-3 rounded-full bg-gray-200 dark:bg-gray-700" /></div>
      ...
      <div className="card-base px-8 py-6 onload-animation">
        {groups?.length ? (
          groups.map((group) => (
            <div key={group.year}>
              <div className="flex flex-row w-full items-center h-[3.75rem]">
                <div className="w-[15%] md:w-[10%] text-2xl font-bold text-right text-75">{group.year}</div>
                ...
                <div className="w-[70%] md:w-[80%] text-left text-50">{group.count} 篇文章</div>
              </div>
              {group.posts.map((post) => (
                <Link key={post.id} to={`/articles/${post.slug}`} aria-label={post.title} className="group btn-plain !block h-10 w-full rounded-lg hover:text-[initial]">
