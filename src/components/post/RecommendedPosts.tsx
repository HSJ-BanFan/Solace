...
// 配置项
const CONFIG: Record<RecommendedMode, {
  title: string;
  badge: string;
  icon: string;
}> = {
...
export function RecommendedPosts({ mode, excludeId, limit = 5, className = '' }: RecommendedPostsProps) {
  ...
  const { data: articles, isLoading, error } = mode === 'random' ? randomResult : recentResult;

  // 过滤排除的文章
  const filteredArticles = articles?.filter(a => a.id !== excludeId).slice(0, limit);
  ...
  return (
    <div className={`card-base p-5 md:p-6 flex flex-col ${className}`}>
      ...
      <div className="flex items-center gap-2 pb-3 mb-1 border-b border-dashed border-[var(--line-divider)]">
        <SafeIcon icon={config.icon} size="1.25rem" className="text-[var(--primary)]" />
...
function PostListItem({
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-30 truncate mt-1">
            ...
            <span className="truncate">{showSummary ? summary : formattedDate}</span>
          </div>
        </div>
      ...
      {!isLast && <div className="border-b border-dashed border-[var(--line-divider)] mx-3" />}
    </>
  );
}
