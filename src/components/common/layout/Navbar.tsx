...
function MenuItem({ link, onClose, onLogout }: { link: { name: string; path: string; icon?: string }; onClose: () => void; onLogout: () => void }) {
	...
	if (isLogout) {
		return (
			<button onClick={onLogout} className="w-full group flex justify-between items-center py-2 pl-3 pr-1 rounded-[var(--radius-small)] hover:bg-[var(--btn-plain-bg-hover)] transition text-left">
				{content}
			</button>
		);
	}
	...
	return (
		<Link to={link.path} onClick={onClose} className="group flex justify-between items-center py-2 pl-3 pr-1 rounded-[var(--radius-small)] hover:bg-[var(--btn-plain-bg-hover)] transition">
			{content}
		</Link>
	);
}
...
<div className="card-base !overflow-visible h-[4.5rem] !rounded-t-none flex items-center justify-between px-4">
			...
			<div className="hidden md:flex">
				{navLinks.map((l) => (
					<Link key={l.path} to={l.path} className="btn-plain scale-animation rounded-[var(--radius-small)] h-11 font-bold px-5">
						{l.name}
					</Link>
				))}
				{isAuthenticated && (
					<Link to="/admin" className="btn-plain scale-animation rounded-[var(--radius-small)] h-11 font-bold px-5">
						管理后台
					</Link>
				)}
			</div>
			...
			<div className="flex items-center gap-1">
				<button onClick={() => setShowSearch(true)} className="btn-plain scale-animation rounded-[var(--radius-small)] h-11 w-11" aria-label="搜索">
					<SafeIcon icon="material-symbols:search-rounded" size="1.25rem" />
				</button>

				<div ref={huePickerRef} className="relative">
					<button onClick={() => setShowHuePicker(!showHuePicker)} className="btn-plain scale-animation rounded-[var(--radius-small)] h-11 w-11" aria-label="显示设置">
				...
				<div className="hidden md:block relative">
					{isAuthenticated ? (
						...
						<Link to="/login" className="btn-regular px-4 h-9 font-medium text-sm flex items-center gap-1.5">
				...
				<button ref={mobileMenuBtnRef} onClick={() => setShowMobileMenu(!showMobileMenu)} className="btn-plain scale-animation rounded-[var(--radius-small)] w-11 h-11 md:hidden" aria-label="菜单">
