...
export function HuePicker({ isOpen }: { isOpen: boolean }) {
  ...
  return (
    <div className={`float-panel absolute right-0 top-12 w-72 px-4 py-3 z-50 transition-all ${isOpen ? '' : 'float-panel-closed'}`}>
      ...
      <div className="w-full h-5 px-1 rounded select-none">
        <input type="range" min="0" max="360" step="5" value={hue} onChange={(e) => setHue(Number(e.target.value))} className="hue-slider w-full cursor-pointer" aria-label="主题颜色色相" />
      </div>
    </div>
  );
}
