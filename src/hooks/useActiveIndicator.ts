...
export function useActiveIndicator({ activeId, containerRef }: UseActiveIndicatorOptions) {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      const navElement = containerRef.current.querySelector('nav') as HTMLElement;
      ...
      if (activeElement && navElement) {
        ...
        // 滚动 TOC 容器使活动项可见
        const scrollContainer = containerRef.current.querySelector('.toc-scroll-container');
        if (scrollContainer) {
