...
func (r *fakeMomentRepository) FindByID(ctx context.Context, id uint) (*model.Moment, error) {
	return nil, gorm.ErrRecordNotFound
}

func (r *fakeMomentRepository) FindAll(ctx context.Context, limit, offset int) ([]*model.Moment, int64, error) {
	return nil, 0, nil
}

func (r *fakeMomentRepository) GetContributions(ctx context.Context, from, to time.Time) ([]*model.Moment, error) {
	return nil, r.contributionsErr
}

func (r *fakeMomentRepository) Create(ctx context.Context, moment *model.Moment) error {
	return nil
}

func (r *fakeMomentRepository) Delete(ctx context.Context, id uint) error {
	return nil
}

func TestArticleServiceCreateSetsAuthorIDFromParameter(t *testing.T) {
	repo := &fakeArticleRepository{}
	service := NewArticleService(repo, &fakeCategoryRepository{}, &fakeTagRepository{}, nil)
...
func TestArticleServiceCreateRejectsMissingAuthorID(t *testing.T) {
	service := NewArticleService(&fakeArticleRepository{}, &fakeCategoryRepository{}, &fakeTagRepository{}, nil)
...
func TestArticleServiceGetContributionsSkipsNilMomentRepo(t *testing.T) {
	service := NewArticleService(&fakeArticleRepository{}, &fakeCategoryRepository{}, &fakeTagRepository{}, nil)

	resp, err := service.GetContributions(context.Background())
	if err != nil {
		t.Fatalf("GetContributions() error = %v", err)
	}
	if resp.Total != 0 {
		t.Fatalf("GetContributions() total = %d, want 0", resp.Total)
	}
	if len(resp.Groups) != 0 {
		t.Fatalf("GetContributions() groups = %d, want 0", len(resp.Groups))
	}
}

func TestArticleServiceGetContributionsReturnsMomentRepoError(t *testing.T) {
	wantErr := errors.New("moment contributions failed")
	service := NewArticleService(&fakeArticleRepository{}, &fakeCategoryRepository{}, &fakeTagRepository{}, &fakeMomentRepository{contributionsErr: wantErr})

	_, err := service.GetContributions(context.Background())
	if !errors.Is(err, wantErr) {
		t.Fatalf("GetContributions() error = %v, want %v", err, wantErr)
	}
}
