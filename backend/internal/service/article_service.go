...
func (s *articleService) Create(ctx context.Context, authorID uint, title, articleSlug, content, summary, coverImage string, categoryID *uint, tagIDs []uint, status string) (*response.ArticleResponse, error) {
	...
	createArticle := func() error {
		if len(tagIDs) > 0 {
			return s.articleRepo.CreateWithTags(ctx, article, tagIDs)
		}
		return s.articleRepo.Create(ctx, article)
	}

	if err := createArticle(); err != nil {
		if autoGeneratedSlug && isArticleSlugConflict(err) {
			finalSlug = slug.GenerateWithTimestamp(title)
			article.Slug = finalSlug
			if retryErr := createArticle(); retryErr != nil {
				log.Error().Err(retryErr).Msg("创建文章失败")
				return nil, retryErr
			}
		} else {
